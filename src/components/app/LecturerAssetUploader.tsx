...
import { createLecturerAssetUploadUrl, recordLecturerAsset } from "@/app/(app)/admin/asset-actions";
...
export function LecturerAssetUploader({ lecturerId, programs }: { lecturerId: string; programs: { id: string; name: string }[] }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [pending, start] = useTransition();
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [kind, setKind] = useState("feedback_proof");
  ...
  async function submit() {
    ...
    if (mode === "link") {
      ...
      start(async () => {
        const res = await recordLecturerAsset({ lecturerId, path: null, name: title || linkUrl, type: null, size: null, asset_kind: kind, program_id: programId || null, title, description, source: "link", link_url: linkUrl.trim() });
    ...
    setBusy(true);
    try {
      const signed = await createLecturerAssetUploadUrl({ lecturerId, filename: file.name });
      if (signed.error || !signed.path || !signed.token) { setError(signed.error || "Could not start upload."); return; }
      const supabase = createClient();
      const { error: upErr } = await supabase.storage.from("lecturer-files").uploadToSignedUrl(signed.path, signed.token, file);
      if (upErr) { setError("Upload failed."); return; }
      const res = await recordLecturerAsset({ lecturerId, path: signed.path, name: file.name, type: file.type, size: file.size, asset_kind: kind, program_id: programId || null, title: title || file.name, description, source: "file", link_url: null });
      if (res.error) setError(res.error); else { reset(); router.refresh(); }
  ...
  return (
    <div className="space-y-3 rounded-card border border-line bg-card p-5 shadow-card">
      <h3 className="text-sm font-semibold text-ink">Add an asset for this instructor</h3>
